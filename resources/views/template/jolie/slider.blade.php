...
<section class="relative w-full min-h-[500px] lg:min-h-[700px] overflow-hidden" style="background-color: {{ $background_color }}">
    ...
    @foreach($repeater as $i => $item)
        @php
            ...
            $slideTag = $item['label'] ?? '';
            $slideTagColor = $item['color'] ?? '#ffffff';
            $slideTitle = $item['title'] ?? '';
            $slideTitleColor = $item['title_color'] ?? '#ffffff';
            $slideSubtitle = $item['subtitle'] ?? '' ;
            $slideBtnText = $item['button_text'] ?? '';
            $slideBtnColor = $item['button_color'] ?? '#000000';
            $slideBtnTextColor = $item['button_text_color'] ?? '#ffffff';
        @endphp
        <div class="slide-item {{ $i === 0 ? 'active-slide' : 'hidden' }} absolute inset-0 w-full h-full">
            ...
            {{-- Content --}}
            <div class="relative z-10 flex flex-col items-center justify-center h-full min-h-[500px] lg:min-h-[700px] px-8 text-center">
                @if($slideTag)
                ...
                @endif
                @if($slideSubtitle)
                    <p class="text-white/80 text-sm md:text-lg leading-relaxed mb-10 max-w-xl">
                        {{ $slideSubtitle }}
                    </p>
                @endif
                @if($slideBtnText)
                    <a href="#"
                        class="inline-block text-sm md:text-base font-semibold px-10 py-4 rounded-full transition duration-300 shadow-lg hover:scale-105"
