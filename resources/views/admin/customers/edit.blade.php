<x-app-layout>
    ...
    <form method="POST" action="{{ route('admin.' . $modul . '.update', $lead->id) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                    ...
                    <div class="space-y-4 p-4 sm:p-5">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <label class="block space-y-3">
                                ...
                                <select name="customer_type" class="form-select mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:bg-navy-700 dark:hover:border-navy-400 dark:focus:border-accent">
                                    <option value="">-- Select Customer Type --</option>
                                    @foreach(['Free Edition', 'Starter Edition', 'Standar Edition', 'Business Edition', 'Enterprise Edition'] as $customerType)
                                        <option value="{{ $customerType }}" {{ old('customer_type', $lead->customer_type) == $customerType ? 'selected' : '' }}>{{ $customerType }}</option>
                                    @endforeach
                                </select>
                            </label>
                        </div>
                ...
                <div class="card">
                    ...
                    <div class="p-4 sm:p-5 space-y-4">
                        <label class="block space-y-1.5 w-full">
                            ...
                            <x-input name="instagram" value="{{ old('instagram', $lead->instagram) }}" placeholder="https://instagram.com/username" autocomplete="off" />
                        </label>
                        <label class="block space-y-1.5 w-full">
                            <span>TikTok URL</span>
                            <x-input name="tiktok" value="{{ old('tiktok', $lead->tiktok) }}" placeholder="https://tiktok.com/@username" autocomplete="off" />
                        </label>
                        <label class="block space-y-1.5 w-full">
                            <span>Facebook URL</span>
                            <x-input name="facebook" value="{{ old('facebook', $lead->facebook) }}" placeholder="https://facebook.com/username" autocomplete="off" />
                        </label>
                        <label class="block space-y-1.5 w-full">
                            <span>X (Twitter) URL</span>
                            <x-input name="x" value="{{ old('x', $lead->x) }}" placeholder="https://x.com/username" autocomplete="off" />
                        </label>
                        <label class="block space-y-1.5 w-full">
                            <span>Threads URL</span>
                            <x-input name="threads" value="{{ old('threads', $lead->threads) }}" placeholder="https://threads.net/@username" autocomplete="off" />
                        </label>
                        <label class="block space-y-1.5 w-full">
                            <span>Shopee URL</span>
                            <x-input name="shopee" value="{{ old('shopee', $lead->shopee) }}" placeholder="https://shopee.co.id/shopname" autocomplete="off" />
                        </label>
                        <label class="block space-y-1.5 w-full">
                            <span>Tokopedia URL</span>
                            <x-input name="tokopedia" value="{{ old('tokopedia', $lead->tokopedia) }}" placeholder="https://tokopedia.com/shopname" autocomplete="off" />
                        </label>
                    </div>
                </div>
            </div>
