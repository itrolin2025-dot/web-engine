<x-app-layout>
    ...
    <form method="POST" action="{{ route('admin.' . $modul . '.update', $transaction->id) }}">
        @csrf
        @method('PUT')
                    ...
                    <div class="space-y-4 p-4 sm:p-5">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <label class="block space-y-1.5 w-full">
                                <span>Code</span>
                                <span class="text-xs ml-2" style="color:red;">(Cannot be changed)</span>
                                <x-input name="code" value="{{ old('code', $transaction->code) }}" autocomplete="off" readonly
                            ...
                            <label class="block space-y-1.5 w-full">
                                ...
                                <select name="customers_website_id" id="website-select" required
                                    ...
                                    @foreach($customers_websites as $w)
                                        <option value="{{ $w->id }}" {{ old('customers_website_id', $transaction->customers_website_id) == $w->id ? 'selected' : '' }}>
                                            {{ $w->title }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            <label class="block space-y-1.5 w-full">
                                ...
                                <x-input name="customer_name" id="customer-name" value="{{ old('customer_name', $transaction->customer_name) }}"
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            <label class="block space-y-1.5 w-full">
                                ...
                                <textarea rows="3" name="customer_address" id="customer-address" required
                                    ...
                                    class="form-textarea w-full rounded-lg border border-slate-300 bg-transparent p-2.5 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent">{{ old('customer_address', $transaction->customer_address) }}</textarea>
                            </label>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    ...
                    <div class="p-4 sm:p-5">
                        <p class="text-xs text-slate-400 mb-3">
                            ...
                            Mengubah website akan memuat ulang daftar produk dan mengosongkan detail barang.
                            Harga bisa diubah (mis. untuk diskon).
                        </p>
                    ...
                    <div class="space-y-4 p-4 sm:p-5">
                        <label class="block space-y-1.5 w-full">
                            ...
                            <textarea rows="3" name="shipping_address" id="shipping-address"
                                ...
                                class="form-textarea w-full rounded-lg border border-slate-300 bg-transparent p-2.5 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent">{{ old('shipping_address', $transaction->shipping_address) }}</textarea>
                        </label>
                        ...
                        <label class="block space-y-1.5 w-full">
                            ...
                            <select name="shipping_courier"
                                ...
                                @foreach(['JNE', 'J&T Express', 'SiCepat', 'AnterAja', 'GoSend', 'GrabExpress', 'Ambil Sendiri', 'Lainnya'] as $courier)
                                    <option value="{{ $courier }}" {{ old('shipping_courier', $transaction->shipping_courier) == $courier ? 'selected' : '' }}>{{ $courier }}</option>
                                @endforeach
                            </select>
                        </label>
                        ...
                        <label class="block space-y-1.5 w-full">
                            ...
                            <x-input name="shipping_tracking_number" value="{{ old('shipping_tracking_number', $transaction->shipping_tracking_number) }}"
                        ...
                        <label class="block space-y-1.5 w-full">
                            ...
                            <x-input type="number" step="0.01" min="0" name="shipping_cost" id="shipping-cost"
                                value="{{ old('shipping_cost', $transaction->shipping_cost) }}" placeholder="0" autocomplete="off"
                        ...
                        <label class="block space-y-1.5 w-full">
                            ...
                            <select name="shipping_status"
                                ...
                                @foreach(['Pending', 'Packing', 'Shipped', 'Delivered'] as $st)
                                    <option value="{{ $st }}" {{ old('shipping_status', $transaction->shipping_status) == $st ? 'selected' : '' }}>{{ $st }}</option>
                                @endforeach
                            </select>
                        </label>
                    </div>
    ...
    @push('scripts')
        <script>
            var websiteProducts = @json($websiteProducts);
            var existingItems = @json($existingItems);
            var itemRowIndex = 0;
            var shippingAddressTouched = false;

            function formatRp(num) {
                return 'Rp ' + Number(num || 0).toLocaleString('id-ID');
            }

            // ===== Load customer info + products saat website diubah =====
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                // Render detail barang yang sudah tersimpan
            ...
            function findProductOption(productId, fallbackName, fallbackPrice) {
                for (var i = 0; i < websiteProducts.length; i++) {
                    if (websiteProducts[i].id == productId) return websiteProducts[i];
                }
                // Produk tidak ada lagi di daftar (dihapus) — tetap tampilkan snapshot
                return productId ? { id: productId, name: fallbackName || 'Produk #' + productId, price: fallbackPrice || 0 } : null;
            }
            ...
            function productOptions(selectedProduct) {
                ...
                websiteProducts.forEach(function (p) {
                    var sel = (selectedProduct && selectedProduct.product_id == p.id) ? ' selected' : '';
                    if (sel) matched = true;
                    html += '<option value="' + p.id + '" data-price="' + p.price + '"' + sel + '>' +
                        p.name + ' — ' + formatRp(p.price) + '</option>';
                });
                // Snapshot produk yang sudah tidak ada di daftar website
                if (selectedProduct && selectedProduct.product_id && !matched) {
                    html += '<option value="' + selectedProduct.product_id + '" data-price="' + selectedProduct.price + '" selected>' +
                        selectedProduct.product_name + ' — ' + formatRp(selectedProduct.price) + '</option>';
                }
                return html;
            }
