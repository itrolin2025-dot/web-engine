<x-app-layout>
    ...
    @php
        // Identity bisa null (website lama belum punya row identity) — semua akses harus null-safe
        ...
        $idColor = [
            ...
            'font_color'      => $identity->font_color ?? null,
            'primary_font'    => $identity->primary_font ?? null,
            'secondary_font'  => $identity->secondary_font ?? null,
            'logo'            => $identity->logo ?? null,
        ];
    @endphp

    <div class="max-w-6xl">
        <form action="{{ route('admin.customers-website.update', $website->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')

            <div class="grid grid-cols-1 items-start gap-5 lg:grid-cols-2">
                <!-- ==================== COLUMN KIRI: Edit Customer Website ==================== -->
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="rounded-xl border border-slate-200 dark:border-navy-500 p-4">
                            <div class="flex flex-col sm:flex-row sm:items-start gap-4">
                                <div class="sm:w-40 shrink-0">
                                    <label for="qr-payment-input"
                                        ...
                                        <img id="qr-payment-preview"
                                            ...
                                            src="{{ $website->qr_payment ? asset($website->qr_payment) : asset('images/200x200.png') }}"
                                            alt="QR Payment preview" />
                                        <span class="flex items-center gap-1.5 text-xs font-medium text-primary dark:text-accent-light">
                                            <i class="fa-solid fa-qrcode"></i>
                                            {{ $website->qr_payment ? 'Ganti QR Payment' : 'Upload QR Payment' }}
                                        </span>
                                ...
                                <div class="text-xs text-slate-500 dark:text-navy-200 space-y-1">
                                    ...
                                    @if($website->qr_payment)
                                        <label class="inline-flex items-center gap-2 cursor-pointer pt-1">
                                            <input type="checkbox" name="remove_qr_payment" value="1"
                                                class="form-checkbox is-outline h-4 w-4 rounded border-slate-300 text-error dark:border-navy-450"
                                                onchange="toggleQrRemove(this)">
                                            <span class="font-medium text-error">Hapus QR payment yang tersimpan</span>
                                        </label>
                                    @endif
                                </div>
                            </div>
                        ...
                        <div class="rounded-xl border border-slate-200 dark:border-navy-500 p-4 space-y-4">
                            ...
                            <label class="block">
                                ...
                                <input name="instagram" value="{{ old('instagram', $website->instagram) }}" placeholder="https://instagram.com/username" autocomplete="off"
                                    class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                    type="text">
                            </label>
                            ...
                            <label class="block">
                                ...
                                <input name="tiktok" value="{{ old('tiktok', $website->tiktok) }}" placeholder="https://tiktok.com/@username" autocomplete="off"
                                    class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                    type="text">
                            </label>
                            ...
                            <label class="block">
                                ...
                                <input name="facebook" value="{{ old('facebook', $website->facebook) }}" placeholder="https://facebook.com/username" autocomplete="off"
                                    class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                    type="text">
                            </label>
                            ...
                            <label class="block">
                                ...
                                <input name="x" value="{{ old('x', $website->x) }}" placeholder="https://x.com/username" autocomplete="off"
                                    class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                    type="text">
                            </label>
                            ...
                            <label class="block">
                                ...
                                <input name="threads" value="{{ old('threads', $website->threads) }}" placeholder="https://threads.net/@username" autocomplete="off"
                                    class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                    type="text">
                            </label>
                            ...
                            <label class="block">
                                ...
                                <input name="shopee" value="{{ old('shopee', $website->shopee) }}" placeholder="https://shopee.co.id/shopname" autocomplete="off"
                                    class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                    type="text">
                            </label>
                            ...
                            <label class="block">
                                ...
                                <input name="tokopedia" value="{{ old('tokopedia', $website->tokopedia) }}" placeholder="https://tokopedia.com/shopname" autocomplete="off"
                                    class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                    type="text">
                            </label>
                        </div>
                    ...
                    <div class="space-y-4">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-start gap-4">
                                <div class="sm:w-40 shrink-0">
                                    <label for="logo-input"
                                        ...
                                        <img id="logo-preview"
                                            class="h-24 w-24 rounded-lg border border-slate-200 bg-white object-contain p-1 transition-transform group-hover:scale-105 dark:border-navy-500"
                                            src="{{ $idColor['logo'] ? asset($idColor['logo']) : asset('images/200x200.png') }}"
                                            alt="Logo preview" />
                                        <span class="flex items-center gap-1.5 text-xs font-medium text-primary dark:text-accent-light">
                                            <i class="fa-solid fa-image"></i>
                                            {{ $idColor['logo'] ? 'Ganti Logo' : 'Upload Logo' }}
                                        </span>
                                    ...
                                    @if($idColor['logo'])
                                        <label class="inline-flex items-center gap-2 cursor-pointer pt-1">
                                            <input type="checkbox" name="remove_logo" value="1"
                                                class="form-checkbox is-outline h-4 w-4 rounded border-slate-300 text-error dark:border-navy-450"
                                                onchange="toggleLogoRemove(this)">
                                            <span class="font-medium text-error">Hapus logo yang tersimpan</span>
                                        </label>
                                    @endif
                                </div>
                            </div>
                        ...
                        <div class="rounded-xl border border-slate-200 dark:border-navy-500 p-4 space-y-4">
                            ...
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                                ...
                                <label class="block">
                                    ...
                                    <span class="mt-1.5 flex items-center gap-2">
                                        <input type="color" value="{{ old('primary_color', $idColor['primary_color'] ?? '#4F46E5') }}" data-color-sync="primary_color"
                                        ...
                                        <input name="primary_color" value="{{ old('primary_color', $idColor['primary_color']) }}" placeholder="#4F46E5" autocomplete="off"
                                ...
                                <label class="block">
                                    ...
                                    <span class="mt-1.5 flex items-center gap-2">
                                        <input type="color" value="{{ old('secondary_color', $idColor['secondary_color'] ?? '#0EA5E9') }}" data-color-sync="secondary_color"
                                            class="h-10 w-12 shrink-0 cursor-pointer rounded-lg border border-slate-300 bg-white p-1 dark:border-navy-450 dark:bg-navy-700">
                                        <input name="secondary_color" value="{{ old('secondary_color', $idColor['secondary_color']) }}" placeholder="#0EA5E9" autocomplete="off"
                                ...
                                <label class="block">
                                    ...
                                    <span class="mt-1.5 flex items-center gap-2">
                                        <input type="color" value="{{ old('accent_color', $idColor['accent_color'] ?? '#F59E0B') }}" data-color-sync="accent_color"
                                            class="h-10 w-12 shrink-0 cursor-pointer rounded-lg border border-slate-300 bg-white p-1 dark:border-navy-450 dark:bg-navy-700">
                                        <input name="accent_color" value="{{ old('accent_color', $idColor['accent_color']) }}" placeholder="#F59E0B" autocomplete="off"
                            ...
                            <label class="block">
                                ...
                                <span class="mt-1.5 flex items-center gap-2">
                                    <input type="color" value="{{ old('font_color', $idColor['font_color'] ?? '#0F172A') }}" data-color-sync="font_color"
                                        class="h-10 w-12 shrink-0 cursor-pointer rounded-lg border border-slate-300 bg-white p-1 dark:border-navy-450 dark:bg-navy-700">
                                    <input name="font_color" value="{{ old('font_color', $idColor['font_color']) }}" placeholder="#0F172A" autocomplete="off"
                            ...
                            <div class="grid grid-cols-1 items-end gap-4 gap-x-3 sm:grid-cols-2">
                                ...
                                <label class="block">
                                    ...
                                    <select name="primary_font" class="form-select mt-1.5 w-full rounded-lg border border-slate-300 bg-white dark:bg-navy-700 px-3 py-2 text-sm hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent">
                                        ...
                                        @foreach(['Inter','Poppins','Roboto','Open Sans','Lato','Montserrat','Playfair Display','Raleway','Nunito','DM Sans','Plus Jakarta Sans'] as $font)
                                            <option value="{{ $font }}" {{ old('primary_font', $idColor['primary_font']) == $font ? 'selected' : '' }}>{{ $font }}</option>
                                        @endforeach
                                    </select>
                                    @error('primary_font')
                                        <span class="text-xs text-error mt-1">{{ $message }}</span>
                                ...
                                <label class="block">
                                    ...
                                    <select name="secondary_font" class="form-select mt-1.5 w-full rounded-lg border border-slate-300 bg-white dark:bg-navy-700 px-3 py-2 text-sm hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent">
                                        <option value="">-- Default --</option>
                                        @foreach(['Inter','Poppins','Roboto','Open Sans','Lato','Montserrat','Playfair Display','Raleway','Nunito','DM Sans','Plus Jakarta Sans'] as $font)
                                            <option value="{{ $font }}" {{ old('secondary_font', $idColor['secondary_font']) == $font ? 'selected' : '' }}>{{ $font }}</option>
                                        @endforeach
                                    </select>
    ...
    @push('scripts')
        <script>
            function previewQrPayment(event) {
                ...
                const removeCb = document.querySelector('input[name="remove_qr_payment"]');

                if (!file) {
                    preview.src = "{{ $website->qr_payment ? asset($website->qr_payment) : asset('images/200x200.png') }}";
                ...
                // Upload baru membatalkan penandaan hapus
                if (removeCb) removeCb.checked = false;
            }

            function toggleQrRemove(cb) {
                const preview = document.getElementById('qr-payment-preview');
                const fileInput = document.getElementById('qr-payment-input');
                if (cb.checked) {
            ...
            function previewLogo(event) {
                ...
                const removeCb = document.querySelector('input[name="remove_logo"]');
                const originalSrc = "{{ $idColor['logo'] ? asset($idColor['logo']) : asset('images/200x200.png') }}";
